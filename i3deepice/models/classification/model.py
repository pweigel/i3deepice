...
from ...lib import block_units as bunit
...
def inception_block4(input_tensor, n, t0=2, t1=4, t2=5, n_pool=3, scale=0.1):
    ...
    tower_3 = keras.layers.MaxPooling3D(
        (n_pool, n_pool, n_pool), strides=(1, 1, 1), padding="same"
    )(input_tensor)
...
def inception_resnet(input_tensor, n, t1=2, t2=3, n_pool=3, scale=0.1):
    ...
    up = keras.layers.concatenate([tower_1, tower_2, tower_3], axis=channel_axis)
...
def model(input_shapes, output_shapes):
    ...
    input_b1 = keras.layers.Input(
        shape=input_shapes["Branch_IC_time"]["general"], name="Input-Branch1"
    )
    ...
    for i in range(8):
        ...
        z1 = inception_resnet(z1, 32, t2=4)
        z1 = inception_resnet(z1, 32, t2=5)
    z1 = keras.layers.Conv3D(
        4096, (1, 1, 1), activation="relu", padding="same", name="conv1x1x1"
    )(z1)
    z1 = keras.layers.GlobalAveragePooling3D()(z1)
    print("out shape {}".format(output_shapes["Out1"]["general"][0]))
    output_b1 = keras.layers.Dense(
        output_shapes["Out1"]["general"][0], activation="softmax", name="Target1"
    )(z1)
    # The Output
